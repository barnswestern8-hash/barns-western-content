...
## Produção Visual — Produto em Fundo Ouro (#FFBF1A)

- Fotos de produto com fundo branco sobre fundo ouro: usar `background-blend-mode: multiply` no container (`#product-area`), com `background-image` injetado via JS, e `background-color: #FFBF1A`. O fundo branco some, o produto fica visível.
- Não funciona se o produto for injetado como `<img>` dentro de container com `z-index` (cria stacking context isolado que impede a mistura com o background da page).
- Técnica validada nos slides `feed-slide-5-produto` e `stories-frame-4` do Run #005.

## Skill de Copywriting — Instrução Permanente do Usuário

- A skill de copywriting (`skills/copywriting/SKILL.md`) deve ser injetada em todos os agentes que geram copy: Fernanda Feed, Rodrigo Reels, Sofia Stories. Instrução dada em 2026-05-03 e vale para todos os runs futuros.
